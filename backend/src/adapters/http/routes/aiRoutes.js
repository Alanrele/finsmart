  ...
  try {
    const status = {
      openai: {
        ...
        available: aiUseCases.isAvailable(),
...
router.post('/analyze', async (req, res) => {
  try {
    ...
    let analysis;
    try {
      analysis = await aiUseCases.generateFinancialAnalysis(transactions, period);
    } catch (aiError) {
...
router.post('/chat', [
  ...
  try {
    ...
    const aiText = await aiUseCases.generateChatResponse(message, recentTransactions);
...
router.get('/insights', async (req, res) => {
  try {
    ...
    try {
      ...
      try {
        insights = await aiUseCases.generateSpendingInsights(trends, monthlyData);
      } catch (aiError) {
...
router.get('/recommendations', async (req, res) => {
  try {
    ...
    try {
      ...
      try {
        recommendations = await aiUseCases.generateRecommendations(transactions);
      } catch (aiError) {
...
router.post('/categorize', async (req, res) => {
  try {
    ...
    for (const transaction of uncategorizedTransactions) {
      try {
        const categoryData = await aiUseCases.categorizeTransaction(transaction.rawText);
