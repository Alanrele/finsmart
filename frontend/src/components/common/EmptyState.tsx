...
const EmptyState = ({ icon: Icon, title, message, action }: EmptyStateProps) => (
  <div className="flex flex-col items-center text-center py-12 px-4">
    <div className="relative mb-5">
      ...
      <span className="absolute left-1/2 -top-4 -translate-x-1/2 h-4 w-0.5 bg-base" aria-hidden="true" />
    ...
    <h3 className="text-[15px] font-bold text-main">{title}</h3>
    {message && (
      <p className="mt-1 text-sm text-muted max-w-xs">{message}</p>
    )}
