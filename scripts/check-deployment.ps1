# 🔍 Script de Verificación Rápida - FinSmart Railway (PowerShell)
...
try {
    ...
    Write-Host "⚠️  WARN - API responding without auth (HTTP $($response.StatusCode))" -ForegroundColor DarkYellow
} catch {
...
try {
    ...
    if ($_.Exception.Response.StatusCode.value__ -eq 403) {
        ...
        Write-Host "⚠️  WARN - Assets check: $($_.Exception.Message)" -ForegroundColor DarkYellow
    }
}
Write-Host ""
